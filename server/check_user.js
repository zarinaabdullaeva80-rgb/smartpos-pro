...
async function main() {
    ...
    for (const user of result.rows) {
        ...
        console.log(`ID:${user.id} | ${user.username} | role:${user.role} | type:${user.user_type} | license:${user.license_id} | hash_valid:${hashValid} | hash_len:${user.password_hash?.length}`);
    }

    // Try to compare omar's password with common passwords
    const omar = result.rows.find(u => u.username === 'omar');
    if (omar) {
        console.log('\n=== TESTING OMAR PASSWORD ===');
        console.log('Hash:', omar.password_hash);

        const testPasswords = ['omar', '123456', 'password', '1234', 'omar123', '12345678'];
