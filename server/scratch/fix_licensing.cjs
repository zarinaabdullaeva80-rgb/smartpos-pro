...
// Regex to find the corrupted block
const regex = /async function syncToCloud[\s\S]*?router\.get\('\/resolve'/;

const newBlock = `async function syncToCloud(licenseData) {
...
if (regex.test(content)) {
    ...
    console.log('✅ Licensing.js fixed successfully');
} else {
    console.error('❌ Could not find the corrupted block in Licensing.js');
    // Try a more flexible search if the exact match fails
    const fallbackRegex = /async function syncToCloud[\s\S]*?async function deleteFromCloud[\s\S]*?router\.get\('\/resolve'/;
    if (fallbackRegex.test(content)) {
         content = content.replace(fallbackRegex, newBlock);
         fs.writeFileSync(filePath, content, 'utf8');
         console.log('✅ Licensing.js fixed via fallback regex');
    } else {
         console.log('File head:', content.substring(0, 500));
    }
}
