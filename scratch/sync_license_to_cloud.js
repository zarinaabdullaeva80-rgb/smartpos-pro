 ...
 * Использование: node scratch/sync_license_to_cloud.js
 */
import pg from 'pg';
...
async function main() {
    ...
    console.log('   organization_id:', lic.organization_id);

    // 2. Отправка на Railway Cloud через /api/license/sync
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            console.error('❌ Ошибка синхронизации:', data.error || JSON.stringify(data));
        }
    } catch (err) {
        console.error('❌ Сетевая ошибка:', err.message);
    }

    // 3. Проверяем, что облако теперь видит лицензию
    console.log('\n🔎 Проверка на облаке...');
    try {
