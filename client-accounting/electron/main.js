...
function readServerMode() {
    ...
    return 'cloud'; // по умолчанию — облако
}
...
// Auto-setup removed — app works in cloud-only mode (Railway)

// PostgreSQL auto-install removed — app works in cloud-only mode (Railway)
...
async function createWindow() {
    ...
    // Устанавливаем server_url для cloud-режима, если не задан
    mainWindow.webContents.on('did-finish-load', () => {
        const mode = readServerMode();
        if (mode === 'cloud') {
            mainWindow.webContents.executeJavaScript(`
                if (!localStorage.getItem('server_url')) {
                    localStorage.setItem('server_url', '${CLOUD_URL}/api');
                    console.log('[Electron] Cloud server_url set:', '${CLOUD_URL}/api');
                }
            `).catch(e => console.log('[Electron] JS inject error:', e.message));
        }
    });
...
app.on('ready', async () => {
    try {
        ...
        if (mode === 'own' || mode === 'server' || mode === 'hybrid') {
            // Проверяем наличие папки сервера
            ...
            if (serverExists) {
                try {
                    startServer();
                    console.log('[App] Server process started');
                } catch (e) {
                ...
                console.log('[App] Server folder missing — auto-switching to cloud mode');
                writeServerMode('cloud');
                mode = 'cloud';
                serverStatus = 'external';
            }
        } else {
            console.log('[App] Cloud mode, connecting to Railway:', CLOUD_URL);
            serverStatus = 'external';
        }
